...
import { BMFont, BMFontChars, BMFontCommon, BMFontInfo, BMFontKernings, BMFontPage } from './type'
...
export default function toBmfInfo(project: Project, fontFamily = ''): BMFont {
    ...
    let fontSize = style.font.size + Math.min(0, style.shadow.offsetY) + style.stroke.width * 2
    fontSize += 6 // extra padding to account for shadow/stroke rounding (?)
    const common: BMFontCommon = {
        //lineHeight: Math.round(style.font.size * style.font.lineHeight),
        lineHeight: Math.round(fontSize /*style.font.size*/ * style.font.lineHeight),
    ...
    glyphList.forEach((glyph) => {
        ...
        const info = {
            ...
            xoffset: globalAdjustMetric.xOffset + glyph.adjustMetric.xOffset - (isUnEmpty ? glyph.trimOffsetLeft : 0) - (isUnEmpty ? layout.padding : 0),
            yoffset: globalAdjustMetric.yOffset + glyph.adjustMetric.yOffset - (isUnEmpty ? glyph.trimOffsetTop : 0) - (isUnEmpty ? layout.padding : 0),
            xadvance: Math.ceil(glyph.fontWidth) + globalAdjustMetric.xAdvance + glyph.adjustMetric.xAdvance,
            page: 0,
            chnl: 15,
        }
        ...
        if (opentype) {
            glyphList.forEach(({ letter }) => {
                const amount = Math.round(
                    opentype.getKerningValue(opentype.charToGlyphIndex(glyph.letter), opentype.charToGlyphIndex(letter)) * fontScale +
